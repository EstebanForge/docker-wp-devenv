...
run_setup_scripts() {
  ...
  if [ -f "scripts/setup-fedora.sh" ] && [[ "$(uname -s)" == "Linux" ]]; then
    print_success "Running Fedora setup..."
    ./scripts/setup-fedora.sh
  else
    print_warning "Non-Linux system detected. Creating directories manually..."
    mkdir -p src/{wp-core,themes,plugins,uploads,mu-plugins}
    mkdir -p php-conf nginx-conf
    # Set ownership using primary group (macOS compatible)
    chown -R "$USER:$(id -gn)" src/ 2>/dev/null || true
    chown -R "$USER:$(id -gn)" php-conf/ 2>/dev/null || true
    chown -R "$USER:$(id -gn)" nginx-conf/ 2>/dev/null || true
    chmod -R 775 src/
  fi
...
composer require wp-theme/theme-name
...
display_final_instructions() {
  ...
  echo -e "  Install plugins: ${BLUE}composer require wp-plugin/plugin-name${NC}"
